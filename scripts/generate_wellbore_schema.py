...
def main() -> None:
    ...
    fig, ax = plt.subplots(figsize=(3.4, 8.5))
    schema.plot(ax=ax, fontsize=7, xtick=False, dtick=False)
    ax.set_title("")
    ax.set_xlabel("")
    ax.set_ylabel("")
    ax.set_xlim(0.08, 0.92)
    ax.set_ylim(WELL_DEPTH + 40, -40)
    ax.set_axis_off()
    ax.grid(False)
    fig.subplots_adjust(left=0, right=1, top=1, bottom=0)
    fig.savefig(SVG_PATH, format="svg", bbox_inches="tight", pad_inches=0, transparent=True)
    fig.savefig(PNG_PATH, format="png", dpi=240, bbox_inches="tight", pad_inches=0, transparent=True)
    plt.close(fig)
